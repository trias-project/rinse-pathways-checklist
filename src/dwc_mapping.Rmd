...
All available pathway information will be mapped to the six main categories of the [CBD standard](https://www.cbd.int/doc/meetings/sbstta/sbstta-18/official/sbstta-18-09-add1-en.pdf). Vector information for which we have pathway information will be mapped to the subcategories of the CBD standard, but **only in certain cases**.

- For pathways `raw_pathway_accidental` and `raw_pathway_disperal`, we will **not** provide vector information. This is because vectors of accidentally introduced species were not analysed in the present study, due to a lack of reliable data (see Zieritz et al. 2016).
...
- For vectors without pathway information (only for `ornamental` and `industry`), we can only map the information to CBD categories/subcategories when the link is straigtforward, which in this case, is not. In this case, we do not map these vectors to the CBD categories.
...
dispersal | biocontrol | natural_dispersal, unaided | NA
dispersal | industry | natural_dispersal, unaided | NA
dispersal | leisure | natural_dispersal, unaided | NA
dispersal | ornamental | natural_dispersal, unaided | NA
dispersal | research | natural_dispersal, unaided | NA
...
pathway_desc %<>% mutate(description = case_when(
  ...
  pathway == "raw_pathway_dispersal" & !is.na(pathway_value) ~ "natural_dispersal,unaided",
...
Separate `description` when more than two values are provided, i.e. in case of `pathway = accidental` or `pathway = natural_dispersal`:
