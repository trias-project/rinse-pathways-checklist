...
literature_references %<>% 
  rowwise() %<>%
  mutate(references_clean = colon_to_seq(raw_references)) %<>%
  ungroup() %>%
  separate_rows(references_clean, sep = ",")
```
...
```{r}
literature_references %<>% left_join(compiled_references, by = c("references_clean" = "number"))
```
