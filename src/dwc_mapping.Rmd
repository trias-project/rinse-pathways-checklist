...
Keep the link between the `raw_references` and the `identifier` for the mapping of resources in the distribution and extension file.

```{r}
reference_ID <- literature_references %>% select(taxonID, raw_references, identifier)
```
...
```{r}
literature_references %<>% select(-starts_with("raw_"), -references_clean, -full_reference)
```
...
```{r}
pathway_desc %<>% filter(!(is.na(value) & is.na(value2))) 
```

```{r}
with(pathway_desc, which(is.na(value) & !is.na(value2)))
```
