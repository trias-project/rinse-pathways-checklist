...
Generate dataframe `identifier_sequence` with the required columns: 
...
Information for the distributions and dates is contained in eight columns in total: 4 columns for the distribution information (Great Brittain, France, Belgium and the Netherlands in resp. `raw_established_gb`, `raw_established_france`, `raw_established_belgium` and `raw_established_netherlands`) and 4 columns for the first record of the taxon in these countries (resp. `raw_first_record_gb`, `raw_first_record_france`, `raw_first_record_belgium` and `raw_first_record_netherlands`). When a taxon is present in a country, this is indicated by a `Y` and a first record is provided. 

We need to transform the information in these columns into two columns: one with country presence information (`country column`, future `locality`, `locationID` and `countryCode`) and one with the date of first observance (`date` column, future `eventDate`). We do this in three steps:
