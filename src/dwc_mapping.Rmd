...
For pathways with value `raw_pathway_disperal` and `raw_pathway_accidental`, we will not provide vector information. This is because vectors of accidentally introduced species were not analysed in the present study, due to a lack of reliable data (see Zieritz et al. 2016). 

Pathways `raw_pathway_import_escape` and `raw_pathway_import_release` will be mapped to the [CBD standard](https://www.cbd.int/doc/meetings/sbstta/sbstta-18/official/sbstta-18-09-add1-en.pdf), using the main six CBD categories:

RINSE terminology | CBD category
--- |---
accidental | stowaway
...
Vector information **for these pathways** will mapped to the subcategories of the CBD standard, **only** when they map unambigiously to one single term (see also [this issue](https://github.com/trias-project/rinse-pathways-checklist/issues/3):

pathway | value | CBD category | CBD subcategory
--- | --- | --- | ---
import_escape | biocontrol | escape | 
import_escape | industry | escape |
import_escape | leisure | escape | live food and bait
import_escape | ornamental | escape | 
import_escape | research | escape | 
...
pathway_desc %>% mutate(description = case_when(
  ...
  pathway == "raw_pathway_import_escape" & vector == "raw_vector_leisure" & vector_value == "Y" ~ "escape_food_bait",
  pathway == "raw_pathway_import_escape" & vector == "raw_vector_leisure" & is.na(vector_value) ~ "escape",
  pathway == "raw_pathway_import_escape" & vector != "raw_vector_leisure"  ~ "escape"
)) %>% View()
```
